...
GleamDrill holds accounts and study history. If you find a vulnerability, please
...
data, the API's secrets, or the host beyond `/tmp/gleamdrill-run`, or that
...
- **The session token lives in `localStorage`.** A deliberate trade, documented
  at `src/gleamdrill/session.gleam`.
