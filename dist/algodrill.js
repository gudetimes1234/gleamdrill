...
}`]]),new v(`pub fn encode(strs: List(String)) -> String
...
}`]]),new v("pub fn is_valid_sudoku(board: List(List(String))) -> Bool",`pub fn is_valid_sudoku(board: List(List(String))) -> Bool {
  todo
}`,`import gleam/list
import gleam/string
import solution
...
}`))}function EO(Z){if(Z==="nc01_contains_duplicate")return new L(fB());else if(Z==="nc02_valid_anagram")return new L(gB());else if(Z==="nc03_two_sum")return new L(uB());else if(Z==="nc04_group_anagrams")return new L(vB());else if(Z==="nc05_top_k_frequent")return new L(mB());else if(Z==="nc06_product_except_self")return new L(pB());else if(Z==="nc07_longest_consecutive")return new L(cB());else if(Z==="nc08_valid_palindrome")return new L(nB());else if(Z==="nc09_two_sum_sorted")return new L(dB());else if(Z==="nc10_three_sum")return new L(lB());else if(Z==="nc11_container_water")return new L(sB());else if(Z==="nc12_best_time_stock")return new L(iB());else if(Z==="nc13_longest_substring")return new L(rB());else if(Z==="nc14_character_replacement")return new L(aB());else if(Z==="nc15_permutation_in_string")return new L(oB());else if(Z==="nc16_valid_parentheses")return new L(tB());else if(Z==="nc17_min_stack")return new L(eB());else if(Z==="nc18_daily_temperatures")return new L(ZL());else if(Z==="nc19_binary_search")return new L(JL());else if(Z==="nc20_find_min_rotated")return new L(QL());else if(Z==="nc21_search_rotated")return new L(XL());else if(Z==="nc22_encode_decode")return new L(YL());else if(Z==="nc23_valid_sudoku")return new L(KL());else if(Z==="tip01_list_patterns")return new L(cK());else if(Z==="tip02_tail_recursion")return new L(nK());else if(Z==="tip03_fold")return new L(dK());else if(Z==="tip04_frequency_maps")return new L(lK());else if(Z==="tip05_result_chains")return new L(sK());else if(Z==="tip06_option")return new L(iK());else if(Z==="tip07_string_patterns")return new L(rK());else if(Z==="tip08_pipelines")return new L(aK());else if(Z==="tip09_records")return new L(oK());else if(Z==="tip10_set_dedupe")return new L(tK());else return new V0(void 0)}function Z8(Z,J,Q){return new _8(Z,J,z4(Z),F0(Q.solutions,(X)=>{return new Q7(X[0],X[1],X[2])}),gQ,new a(Q.check),z0)}function CO(){return new k8("Gleam Tips",j([new j1("Idioms",j([Z8("Pattern matching on lists","Implement length and last for List(a) using only case expressions with empty, single-element, and head-tail patterns.",cK()),Z8("Tail recursion with accumulators","Implement reverse and sum as tail-recursive functions: a public wrapper plus a private loop helper carrying an accumulator.",nK()),Z8("fold is the loop","Implement max, count_if, and running_total using only list.fold and list.scan - no hand-written recursion.",dK()),Z8("Frequency maps with dict.upsert","Build word_frequencies(text) returning a Dict(String, Int) of word counts: lowercase, split on spaces, drop empties, then fold with dict.upsert.",lK()),Z8("Result chains with use","Define a Config record and parse_config(host, port, timeout) that chains two int.parse calls with use + result.try and rejects an empty host.",sK()),Z8("Option ergonomics",'Write port_description(config) that reads an optional "port" entry from a Dict and produces a display string via option.from_result, option.map, and option.unwrap - no case expression.',iK()),Z8("String prefix patterns and graphemes",'Implement strip_comment using "# " <> rest string-prefix patterns, and initials(name) using string.to_graphemes with list.filter_map.',rK()),Z8("Pipelines","Write slug(title) as a single |> pipeline: trim, lowercase, split on spaces, drop empty words, join with dashes.",aK()),Z8("Records: labelled args and update syntax","Define a Player record and write new_player (labelled construction), add_points, and level_up using Player(..player, field: value) update syntax.",oK()),Z8("gleam/set for membership and dedupe","Implement dedupe(items) preserving first occurrence: fold carrying #(kept, seen_set), then reverse the kept list.",tK())]))]))}class v0 extends P{constructor(Z,J,Q){super();this.title=Z,this.prompt=J,this.stem=Q}}class I4 extends P{constructor(Z,J){super();this.subcategory=Z,this.drills=J}}var j9="NeetCode 150";function WL(){return j([new I4("Arrays & Hashing",j([new v0("Contains Duplicate","Given an integer array nums, return true if any value appears at least twice in the array, and return false if every element is distinct.","nc01_contains_duplicate"),new v0("Valid Anagram","Given two strings s and t, return true if t is an anagram of s, and false otherwise.","nc02_valid_anagram"),new v0("Two Sum","Given an array of integers nums and an integer target, return indices of the two numbers such that they add up to target.","nc03_two_sum"),new v0("Group Anagrams","Given an array of strings strs, group the anagrams together. You can return the answer in any order.","nc04_group_anagrams"),new v0("Top K Frequent Elements","Given an integer array nums and an integer k, return the k most frequent elements. You may return the answer in any order.","nc05_top_k_frequent"),new v0("Encode and Decode Strings","Design an algorithm to encode a list of strings to a single string, and another to decode that string back to the original list. The strings may contain any characters.","nc22_encode_decode"),new v0("Product of Array Except Self","Given an integer array nums, return an array answer such that answer[i] is equal to the product of all the elements of nums except nums[i]. Solve without division and in O(n) time.","nc06_product_except_self"),new v0("Valid Sudoku",'Determine if a 9 x 9 Sudoku board is valid. Only the filled cells need to be validated: each row, each column and each of the nine 3 x 3 sub-boxes must contain the digits 1-9 without repetition. Empty cells are written ".".',"nc23_valid_sudoku"),new v0("Longest Consecutive Sequence","Given an unsorted array of integers nums, return the length of the longest consecutive elements sequence. You must write an algorithm that runs in O(n) time.","nc07_longest_consecutive")])),new I4("Two Pointers",j([new v0("Valid Palindrome","A phrase is a palindrome if, after converting all uppercase letters into lowercase letters and removing all non-alphanumeric characters, it reads the same forward and backward.","nc08_valid_palindrome"),new v0("Two Sum II - Input Array Is Sorted","Given a 1-indexed array of integers numbers that is already sorted in non-decreasing order, find two numbers such that they add up to a specific target number. Return the two indices, 1-indexed.","nc09_two_sum_sorted"),new v0("3Sum","Given an integer array nums, return all the triplets [nums[i], nums[j], nums[k]] such that i, j, and k are distinct and the sum is 0. The solution set must not contain duplicate triplets.","nc10_three_sum"),new v0("Container With Most Water","You are given an integer array height of length n. Find two lines that together with the x-axis form a container that holds the most water. Return the maximum amount of water a container can store.","nc11_container_water")])),new I4("Sliding Window",j([new v0("Best Time to Buy and Sell Stock","You are given an array prices where prices[i] is the price of a given stock on the ith day. Maximize your profit by choosing a single day to buy and a different day in the future to sell.","nc12_best_time_stock"),new v0("Longest Substring Without Repeating Characters","Given a string s, find the length of the longest substring without repeating characters.","nc13_longest_substring"),new v0("Longest Repeating Character Replacement","You are given a string s and an integer k. You can change any character to any other uppercase English character at most k times. Return the length of the longest substring containing the same letter you can get.","nc14_character_replacement"),new v0("Permutation in String","Given two strings s1 and s2, return true if s2 contains a permutation of s1. In other words, return true if one of s1's permutations is a substring of s2.","nc15_permutation_in_string")])),new I4("Stack",j([new v0("Valid Parentheses","Given a string s containing just the characters '(', ')', '{', '}', '[' and ']', determine if the input string is valid.","nc16_valid_parentheses"),new v0("Min Stack","Design a stack that supports push, pop, top, and retrieving the minimum element in constant time.","nc17_min_stack"),new v0("Daily Temperatures","Given an array of integers temperatures, return an array answer such that answer[i] is the number of days you have to wait after the ith day to get a warmer temperature.","nc18_daily_temperatures")])),new I4("Binary Search",j([new v0("Binary Search","Given a sorted array of integers nums and an integer target, write a function to search target in nums. Return its index, or -1 if it does not exist. Must run in O(log n) time.","nc19_binary_search"),new v0("Find Minimum in Rotated Sorted Array","Suppose an array of length n sorted in ascending order is rotated. Given the rotated array of unique elements, return the minimum element. Must run in O(log n) time.","nc20_find_min_rotated"),new v0("Search in Rotated Sorted Array","Given a rotated sorted array of distinct integers nums and an integer target, return the index of target, or -1 if it is not present. Must run in O(log n) time.","nc21_search_rotated")]))])}function GL(Z,J,Q){return F1(Q(Z.stem),(X)=>{let Y=X[0],K=X[1];return new _8(Z.title,Z.prompt,z4(Z.title),F0(Y,(q)=>{return new Q7(q[0],q[1],q[2])}),J,K,z0)})}function H9(Z,J,Q){return new k8(Z,(()=>{let X=WL();return V7(X,(Y)=>{let K=V7(Y.drills,(q)=>{return GL(q,J,Q)});if(K instanceof x)return new V0(void 0);else{let q=K;return new L(new j1(Y.subcategory,q))}})})())}function UL(){return j([["Solution 1","A set answers the whole question. Walk once, and the moment a value is already in the set you are done — no need to finish the pass, and no need to know where the duplicate was.",`defmodule Solution do
...
end`]])}function zL(){return j([["Solution 1","Two strings are anagrams exactly when every character occurs the same number of times in both, so build a count per string and compare the two maps.",`defmodule Solution do
  def anagram?(s, t) do
...
end`]])}function IL(){return j([["Solution 1","Every number seen so far is already in the map, so the complement is one lookup away — one pass, O(1) per step, and the map hands back the index for free.",`defmodule Solution do
...
end`]])}function VL(){return j([["Solution 1",`Bucket by a key that comes out identical for everything that belongs together. Once the key is anagram-invariant the grouping is just a map from key to list, and no pair of words is ever compared directly.
...
end`]])}function jL(){return j([["Solution 1","Count, then select. The frequencies come first; picking the k largest is a separate question, and which method you use for it is what separates the variants.",`defmodule Solution do
...
end`]])}function HL(){return j([["Solution 1","The answer at each slot is everything before it times everything after it. One forward pass builds the prefixes, one reverse pass folds in the suffixes — and no division, so a zero in the input costs nothing special.",`defmodule Solution do
...
end`]])}function OL(){return j([["Solution 1","Put everything in a set, then only start counting at numbers with no predecessor. That guard is what keeps it O(n): every run is walked exactly once instead of once per member.",`defmodule Solution do
...
end`]])}function FL(){return j([["Solution 1","Normalise first — letters and digits only, lowercased — and the palindrome test is whatever comparison you like: two pointers converging, or the cleaned string against its reverse.",`defmodule Solution do
  def palindrome?(s) do
    cleaned = clean(s)
...
end`]])}function NL(){return j([["Solution 1",`Sorted input plus a pointer at each end. A sum that is too small can only be fixed by raising the low end, one that is too large by lowering the high end, so neither pointer ever needs to go back. They meet in O(n) with no extra memory.
...
end`]])}function RL(){return j([["Solution 1","Sort, fix one number, then run the two-pointer scan on the remainder looking for its negation. Sorting is what makes the duplicate triples skippable: equal values are adjacent, so stepping past them is a while loop, not a set.",`defmodule Solution do
...
end`]])}function ML(){return j([["Solution 1","Start at both ends. The area is capped by the shorter line, so moving the taller one in can never help — always move the shorter, and track the best area seen.",`defmodule Solution do
...
end`]])}function AL(){return j([["Solution 1","Carry the cheapest price seen so far; today's best sale is today's price against that minimum. One pass, two variables.",`defmodule Solution do
...
end`]])}function DL(){return j([["Solution 1","Grow a window rightwards and, whenever the new character is already inside it, move the start past that character's earlier copy. The window is always repeat-free, so its widest reading is the answer.",`defmodule Solution do
...
end`]])}function BL(){return j([["Solution 1","A window can be made uniform with k changes when its size minus its most-frequent-character count is at most k. Grow the right edge, shrink from the left when that breaks, and the biggest valid window is the answer.",`defmodule Solution do
...
end`]])}function LL(){return j([["Solution 1","A permutation of s1 is any window of length |s1| in s2 with identical character counts. Slide one character at a time, adding the entering character and removing the leaving one, so each step is O(1) rather than a recount.",`defmodule Solution do
...
end`]])}function PL(){return j([["Solution 1","On every opener, push the closer you expect; on every closer, it must match the top. Valid means never mismatching and finishing with an empty stack — both halves are needed.",`defmodule Solution do
  @closers %{"(" => ")", "[" => "]", "{" => "}"}
...
end`]])}function TL(){return j([["Solution 1","The minimum has to be O(1), so it cannot be computed on demand — it has to be carried. Either each entry remembers the minimum at or below it, or a second stack tracks the running minimum alongside the first.",`defmodule Solution do
...
end`]])}function SL(){return j([["Solution 1","A stack of days still waiting for something warmer, kept in decreasing temperature order. Each new day resolves and pops every colder day below it, so every day is pushed once and popped once — O(n).",`defmodule Solution do
...
end`]])}function kL(){return j([["Solution 1","Length-prefix each string: its length, a delimiter, then the string itself. Decoding reads a number and then takes exactly that many characters, so nothing inside a payload can ever be mistaken for structure — the delimiter appearing in the data is harmless, because the decoder was never scanning for it.",`
...
end`]])}function wL(){return j([["Solution 1","One pass, one set. Each filled cell contributes three signatures — this value in this row, in this column, in this box — and the first one already present is the duplicate. Nothing has to be gathered up first, and the scan stops the moment it fails.",`defmodule Solution do
...
end`]])}function _O(Z){if(Z==="nc01_contains_duplicate")return new L(UL());else if(Z==="nc02_valid_anagram")return new L(zL());else if(Z==="nc03_two_sum")return new L(IL());else if(Z==="nc04_group_anagrams")return new L(VL());else if(Z==="nc05_top_k_frequent")return new L(jL());else if(Z==="nc06_product_except_self")return new L(HL());else if(Z==="nc07_longest_consecutive")return new L(OL());else if(Z==="nc08_valid_palindrome")return new L(FL());else if(Z==="nc09_two_sum_sorted")return new L(NL());else if(Z==="nc10_three_sum")return new L(RL());else if(Z==="nc11_container_water")return new L(ML());else if(Z==="nc12_best_time_stock")return new L(AL());else if(Z==="nc13_longest_substring")return new L(DL());else if(Z==="nc14_character_replacement")return new L(BL());else if(Z==="nc15_permutation_in_string")return new L(LL());else if(Z==="nc16_valid_parentheses")return new L(PL());else if(Z==="nc17_min_stack")return new L(TL());else if(Z==="nc18_daily_temperatures")return new L(SL());else if(Z==="nc19_binary_search")return new L(EL());else if(Z==="nc20_find_min_rotated")return new L(CL());else if(Z==="nc21_search_rotated")return new L(_L());else if(Z==="nc22_encode_decode")return new L(kL());else if(Z==="nc23_valid_sudoku")return new L(wL());else return new V0(void 0)}function kO(){return H9(j9+" (Elixir)",LO,(Z)=>{return F1(_O(Z),(J)=>{return[J,z0]})})}function wO(){return H9(j9+" (Gleam)",gQ,(Z)=>{return F1(EO(Z),(J)=>{return[J.solutions,new a(J.check)]})})}function hL(){return new m(j([["Solution 1","A set answers the whole question. Walk once, and the moment a value is already in the set you are done — no need to finish the pass, and no need to know where the duplicate was.",`def containsDuplicate(nums):
    seen = set()
    for num in nums:
        if num in seen:
            return True
...
__case__("containsDuplicate([])", False, containsDuplicate([]))`))}function $L(){return new m(j([["Solution 1","Two strings are anagrams exactly when every character occurs the same number of times in both, so build a count per string and compare the two maps.",`def isAnagram(s, t):
    if len(s) != len(t):
        return False
...
__case__("isAnagram('a', 'ab')", False, isAnagram("a", "ab"))`))}function fL(){return new m(j([["Solution 1","Every number seen so far is already in the map, so the complement is one lookup away — one pass, O(1) per step, and the map hands back the index for free.",`def twoSum(nums, target):
    seen = {}
...
__case__("twoSum([1, 2], 7)", [], twoSum([1, 2], 7))`))}function gL(){return new m(j([["Solution 1",`Bucket by a key that comes out identical for everything that belongs together. Once the key is anagram-invariant the grouping is just a map from key to list, and no pair of words is ever compared directly.
...
__case__("groupAnagrams(['a'])", [["a"]], sorted(groupAnagrams(["a"])))`))}function uL(){return new m(j([["Solution 1","Count, then select. The frequencies come first; picking the k largest is a separate question, and which method you use for it is what separates the variants.",`from collections import Counter
...
__case__("topKFrequent([5, 5, 4, 4, 4, 3], 1)", [4], topKFrequent([5, 5, 4, 4, 4, 3], 1))`))}function vL(){return new m(j([["Solution 1","The answer at each slot is everything before it times everything after it. One forward pass builds the prefixes, one reverse pass folds in the suffixes — and no division, so a zero in the input costs nothing special.",`def productExceptSelf(nums):
    n = len(nums)
    result = [1] * n
...
__case__("productExceptSelf([2, 3])", [3, 2], productExceptSelf([2, 3]))`))}function mL(){return new m(j([["Solution 1","Put everything in a set, then only start counting at numbers with no predecessor. That guard is what keeps it O(n): every run is walked exactly once instead of once per member.",`def longestConsecutive(nums):
    num_set = set(nums)
    longest = 0
...
__case__("longestConsecutive([])", 0, longestConsecutive([]))`))}function pL(){return new m(j([["Solution 1","Normalise first — letters and digits only, lowercased — and the palindrome test is whatever comparison you like: two pointers converging, or the cleaned string against its reverse.",`def isPalindrome(s):
...
__case__("isPalindrome('0P')", False, isPalindrome("0P"))`))}function cL(){return new m(j([["Solution 1",`Sorted input plus a pointer at each end. A sum that is too small can only be fixed by raising the low end, one that is too large by lowering the high end, so neither pointer ever needs to go back. They meet in O(n) with no extra memory.
...
__case__("twoSum([1, 2, 3], 100)", [], twoSum([1, 2, 3], 100))`))}function nL(){return new m(j([["Solution 1","Sort, fix one number, then run the two-pointer scan on the remainder looking for its negation. Sorting is what makes the duplicate triples skippable: equal values are adjacent, so stepping past them is a while loop, not a set.",`def threeSum(nums):
    nums.sort()
    result = []
...
__case__("threeSum([0, 0, 0])", [[0, 0, 0]], threeSum([0, 0, 0]))`))}function dL(){return new m(j([["Solution 1","Start at both ends. The area is capped by the shorter line, so moving the taller one in can never help — always move the shorter, and track the best area seen.",`def maxArea(height):
...
__case__("maxArea([1, 1])", 1, maxArea([1, 1]))`))}function lL(){return new m(j([["Solution 1","Carry the cheapest price seen so far; today's best sale is today's price against that minimum. One pass, two variables.",`def maxProfit(prices):
    lowest = float('inf')
    profit = 0
...
__case__("maxProfit([])", 0, maxProfit([]))`))}function sL(){return new m(j([["Solution 1","Grow a window rightwards and, whenever the new character is already inside it, move the start past that character's earlier copy. The window is always repeat-free, so its widest reading is the answer.",`def lengthOfLongestSubstring(s):
    window = set()
    left = 0
    longest = 0
...
__case__("lengthOfLongestSubstring('')", 0, lengthOfLongestSubstring(""))`))}function iL(){return new m(j([["Solution 1","A window can be made uniform with k changes when its size minus its most-frequent-character count is at most k. Grow the right edge, shrink from the left when that breaks, and the biggest valid window is the answer.",`def characterReplacement(s, k):
    count = {}
    left = 0
    max_count = 0
    longest = 0
...
__case__("checkInclusion('adc', 'dcda')", True, checkInclusion("adc", "dcda"))`))}function aL(){return new m(j([["Solution 1","On every opener, push the closer you expect; on every closer, it must match the top. Valid means never mismatching and finishing with an empty stack — both halves are needed.",`def isValid(s):
    pairs = {')': '(', ']': '[', '}': '{'}
    stack = []
...
__case__("isValid('([)]')", False, isValid("([)]"))
__case__("isValid('{[]}')", True, isValid("{[]}"))
__case__("isValid('(')", False, isValid("("))`))}function oL(){return new m(j([["Solution 1","The minimum has to be O(1), so it cannot be computed on demand — it has to be carried. Either each entry remembers the minimum at or below it, or a second stack tracks the running minimum alongside the first.",`class MinStack:
    def __init__(self):
        self.stack = []
        self.mins = []
...
__case__("search([], 1)", -1, search([], 1))`))}function ZP(){return new m(j([["Solution 1",`Compare against the midpoint and throw away the half that cannot hold the answer. O(log n); the only thing to get right is which side the midpoint itself falls on, which is what decides whether the loop terminates.
...
__case__("findMin([2, 1])", 1, findMin([2, 1]))`))}function JP(){return new m(j([["Solution 1",`Compare against the midpoint and throw away the half that cannot hold the answer. O(log n); the only thing to get right is which side the midpoint itself falls on, which is what decides whether the loop terminates.
...
__case__("search([4, 5, 6, 7, 0, 1, 2], 6)", 2, search([4, 5, 6, 7, 0, 1, 2], 6))`))}function QP(){return new m(j([["Solution 1","Length-prefix each string: its length, a delimiter, then the string itself. Decoding reads a number and then takes exactly that many characters, so nothing inside a payload can ever be mistaken for structure — the delimiter appearing in the data is harmless, because the decoder was never scanning for it.",`
def encode(strs):
...
def decode(s):
    ...
    while i < len(s):
        if s[i] == ESCAPE:
            ...
            i += 1
    out.append("".join(current))
    return out`]]),new v(`def encode(strs):
...
__case__("decode(encode(['\\\\\\\\', '|', '#']))", ["\\\\", "|", "#"], __round_trip__(["\\\\", "|", "#"]))`))}function XP(){return new m(j([["Solution 1","One pass, one set. Each filled cell contributes three signatures — this value in this row, in this column, in this box — and the first one already present is the duplicate. Nothing has to be gathered up first, and the scan stops the moment it fails.",`def isValidSudoku(board):
    seen = set()
...
def noDuplicates(unit):
    ...
    return len(filled) == len(set(filled))`]]),new v("def isValidSudoku(board):",`def isValidSudoku(board):
    pass`,`try:
    (isValidSudoku)
except NameError:
    raise Exception("__signature_mismatch__")
...
__case__("isValidSudoku(empty board)", True, isValidSudoku([["."] * 9 for _ in range(9)]))`))}function eK(){return new m(j([["Solution 1","",`from collections import Counter
...
__case__("joinUpper([])", "", joinUpper([]))`))}function xO(Z){if(Z==="nc01_contains_duplicate")return new L(hL());else if(Z==="nc02_valid_anagram")return new L($L());else if(Z==="nc03_two_sum")return new L(fL());else if(Z==="nc04_group_anagrams")return new L(gL());else if(Z==="nc05_top_k_frequent")return new L(uL());else if(Z==="nc06_product_except_self")return new L(vL());else if(Z==="nc07_longest_consecutive")return new L(mL());else if(Z==="nc08_valid_palindrome")return new L(pL());else if(Z==="nc09_two_sum_sorted")return new L(cL());else if(Z==="nc10_three_sum")return new L(nL());else if(Z==="nc11_container_water")return new L(dL());else if(Z==="nc12_best_time_stock")return new L(lL());else if(Z==="nc13_longest_substring")return new L(sL());else if(Z==="nc14_character_replacement")return new L(iL());else if(Z==="nc15_permutation_in_string")return new L(rL());else if(Z==="nc16_valid_parentheses")return new L(aL());else if(Z==="nc17_min_stack")return new L(oL());else if(Z==="nc18_daily_temperatures")return new L(tL());else if(Z==="nc19_binary_search")return new L(eL());else if(Z==="nc20_find_min_rotated")return new L(ZP());else if(Z==="nc21_search_rotated")return new L(JP());else if(Z==="nc22_encode_decode")return new L(QP());else if(Z==="nc23_valid_sudoku")return new L(XP());else if(Z==="tip01_counter")return new L(eK());else if(Z==="tip02_defaultdict")return new L(Zq());else if(Z==="tip03_deque")return new L(Jq());else if(Z==="tip04_heapq")return new L(Qq());else if(Z==="tip05_enumerate_zip")return new L(Xq());else if(Z==="tip06_slicing")return new L(Yq());else if(Z==="tip07_sort_key")return new L(Kq());else if(Z==="tip08_join")return new L(qq());else return new V0(void 0)}function yO(){return H9(j9,$Q,(Z)=>{return F1(xO(Z),(J)=>{return[J.solutions,new a(J.check)]})})}function KP(){return new m(j([["Solution 1","A set answers the whole question. Walk once, and the moment a value is already in the set you are done — no need to finish the pass, and no need to know where the duplicate was.",`export function containsDuplicate(nums: number[]): boolean {
...
}`))}function qP(){return new m(j([["Solution 1","Two strings are anagrams exactly when every character occurs the same number of times in both, so build a count per string and compare the two maps.",`export function isAnagram(s: string, t: string): boolean {
...
}`))}function WP(){return new m(j([["Solution 1","Every number seen so far is already in the map, so the complement is one lookup away — one pass, O(1) per step, and the map hands back the index for free.",`export function twoSum(nums: number[], target: number): number[] {
...
}`))}function GP(){return new m(j([["Solution 1",`Bucket by a key that comes out identical for everything that belongs together. Once the key is anagram-invariant the grouping is just a map from key to list, and no pair of words is ever compared directly.
...
}`))}function UP(){return new m(j([["Solution 1","Count, then select. The frequencies come first; picking the k largest is a separate question, and which method you use for it is what separates the variants.",`export function topKFrequent(nums: number[], k: number): number[] {
...
}`))}function IP(){return new m(j([["Solution 1","Put everything in a set, then only start counting at numbers with no predecessor. That guard is what keeps it O(n): every run is walked exactly once instead of once per member.",`export function longestConsecutive(nums: number[]): number {
...
}`))}function VP(){return new m(j([["Solution 1","Normalise first — letters and digits only, lowercased — and the palindrome test is whatever comparison you like: two pointers converging, or the cleaned string against its reverse.",`export function isPalindrome(s: string): boolean {
...
}`))}function jP(){return new m(j([["Solution 1",`Sorted input plus a pointer at each end. A sum that is too small can only be fixed by raising the low end, one that is too large by lowering the high end, so neither pointer ever needs to go back. They meet in O(n) with no extra memory.
...
}`))}function OP(){return new m(j([["Solution 1","Start at both ends. The area is capped by the shorter line, so moving the taller one in can never help — always move the shorter, and track the best area seen.",`export function maxArea(height: number[]): number {
  let left = 0;
...
}`))}function FP(){return new m(j([["Solution 1","Carry the cheapest price seen so far; today's best sale is today's price against that minimum. One pass, two variables.",`export function maxProfit(prices: number[]): number {
  let lowest = Infinity;
  let profit = 0;
...
}`))}function NP(){return new m(j([["Solution 1","Grow a window rightwards and, whenever the new character is already inside it, move the start past that character's earlier copy. The window is always repeat-free, so its widest reading is the answer.",`export function lengthOfLongestSubstring(s: string): number {
...
}`))}function RP(){return new m(j([["Solution 1","A window can be made uniform with k changes when its size minus its most-frequent-character count is at most k. Grow the right edge, shrink from the left when that breaks, and the biggest valid window is the answer.",`export function characterReplacement(s: string, k: number): number {
  const counts = new Map<string, number>();
  let left = 0;
  let maxCount = 0;
  let longest = 0;
...
}`))}function AP(){return new m(j([["Solution 1","On every opener, push the closer you expect; on every closer, it must match the top. Valid means never mismatching and finishing with an empty stack — both halves are needed.",`export function isValid(s: string): boolean {
...
}`))}function DP(){return new m(j([["Solution 1","The minimum has to be O(1), so it cannot be computed on demand — it has to be carried. Either each entry remembers the minimum at or below it, or a second stack tracks the running minimum alongside the first.",`export class MinStack {
...
}`))}function BP(){return new m(j([["Solution 1","A stack of days still waiting for something warmer, kept in decreasing temperature order. Each new day resolves and pops every colder day below it, so every day is pushed once and popped once — O(n).",`export function dailyTemperatures(temperatures: number[]): number[] {
...
}`))}function LP(){return new m(j([["Solution 1",`Compare against the midpoint and throw away the half that cannot hold the answer. O(log n); the only thing to get right is which side the midpoint itself falls on, which is what decides whether the loop terminates.
...
}`))}function PP(){return new m(j([["Solution 1",`Compare against the midpoint and throw away the half that cannot hold the answer. O(log n); the only thing to get right is which side the midpoint itself falls on, which is what decides whether the loop terminates.
...
}`))}function TP(){return new m(j([["Solution 1",`Compare against the midpoint and throw away the half that cannot hold the answer. O(log n); the only thing to get right is which side the midpoint itself falls on, which is what decides whether the loop terminates.
...
}`))}function SP(){return new m(j([["Solution 1","Length-prefix each string: its length, a delimiter, then the string itself. Decoding reads a number and then takes exactly that many characters, so nothing inside a payload can ever be mistaken for structure — the delimiter appearing in the data is harmless, because the decoder was never scanning for it.",`
...
export function decode(s: string): string[] {
  ...
  out.push(current);
  return out;
}`]]),new v(`export function encode(strs: string[]): string
...
}`))}function EP(){return new m(j([["Solution 1","One pass, one set. Each filled cell contributes three signatures — this value in this row, in this column, in this box — and the first one already present is the duplicate. Nothing has to be gathered up first, and the scan stops the moment it fails.",`export function isValidSudoku(board: string[][]): boolean {
  const seen = new Set<string>();
...
}`]]),new v("export function isValidSudoku(board: string[][]): boolean",`export function isValidSudoku(board: string[][]): boolean {
  // todo
}`,`import * as solution from "./solution";
