...
}`))}function gB(){return new p(j([["Solution 1","Two strings are anagrams exactly when every character occurs the same number of times in both, so build a count per string and compare the two maps.",`import gleam/dict
import gleam/list
import gleam/option
import gleam/string
...
}`))}function uB(){return new p(j([["Solution 1","Every number seen so far is already in the map, so the complement is one lookup away — one pass, O(1) per step, and the map hands back the index for free.",`import gleam/dict
...
}`))}function vB(){return new p(j([["Solution 1",`Bucket by a key that comes out identical for everything that belongs together. Once the key is anagram-invariant the grouping is just a map from key to list, and no pair of words is ever compared directly.
...
}`))}function mB(){return new p(j([["Solution 1","Count, then select. The frequencies come first; picking the k largest is a separate question, and which method you use for it is what separates the variants.",`import gleam/dict
import gleam/int
import gleam/list
import gleam/option
...
}`))}function pB(){return new p(j([["Solution 1","The answer at each slot is everything before it times everything after it. One forward pass builds the prefixes, one reverse pass folds in the suffixes — and no division, so a zero in the input costs nothing special.",`import gleam/list
...
}`))}function cB(){return new p(j([["Solution 1","Put everything in a set, then only start counting at numbers with no predecessor. That guard is what keeps it O(n): every run is walked exactly once instead of once per member.",`import gleam/int
import gleam/list
import gleam/set
...
}`))}function nB(){return new p(j([["Solution 1","Normalise first — letters and digits only, lowercased — and the palindrome test is whatever comparison you like: two pointers converging, or the cleaned string against its reverse.",`import gleam/list
import gleam/string
...
}`))}function dB(){return new p(j([["Solution 1",`Sorted input plus a pointer at each end. A sum that is too small can only be fixed by raising the low end, one that is too large by lowering the high end, so neither pointer ever needs to go back. They meet in O(n) with no extra memory.
...
}`))}function lB(){return new p(j([["Solution 1","Sort, fix one number, then run the two-pointer scan on the remainder looking for its negation. Sorting is what makes the duplicate triples skippable: equal values are adjacent, so stepping past them is a while loop, not a set.",`import gleam/int
import gleam/list
import gleam/order
import gleam/set
...
}`))}function sB(){return new p(j([["Solution 1","Start at both ends. The area is capped by the shorter line, so moving the taller one in can never help — always move the shorter, and track the best area seen.",`import gleam/int
import gleam/list
...
}`))}function iB(){return new p(j([["Solution 1","Carry the cheapest price seen so far; today's best sale is today's price against that minimum. One pass, two variables.",`import gleam/int
import gleam/list
...
}`))}function rB(){return new p(j([["Solution 1","Grow a window rightwards and, whenever the new character is already inside it, move the start past that character's earlier copy. The window is always repeat-free, so its widest reading is the answer.",`import gleam/dict
import gleam/int
import gleam/list
import gleam/string
...
}`))}function aB(){return new p(j([["Solution 1","A window can be made uniform with k changes when its size minus its most-frequent-character count is at most k. Grow the right edge, shrink from the left when that breaks, and the biggest valid window is the answer.",`import gleam/dict
import gleam/int
import gleam/list
import gleam/option
import gleam/string
...
}`))}function oB(){return new p(j([["Solution 1","A permutation of s1 is any window of length |s1| in s2 with identical character counts. Slide one character at a time, adding the entering character and removing the leaving one, so each step is O(1) rather than a recount.",`import gleam/dict
import gleam/list
import gleam/option
import gleam/string
...
}`))}function tB(){return new p(j([["Solution 1","On every opener, push the closer you expect; on every closer, it must match the top. Valid means never mismatching and finishing with an empty stack — both halves are needed.",`import gleam/string
...
}`))}function eB(){return new p(j([["Solution 1","The minimum has to be O(1), so it cannot be computed on demand — it has to be carried. Either each entry remembers the minimum at or below it, or a second stack tracks the running minimum alongside the first.",`import gleam/int
...
}`))}function ZL(){return new p(j([["Solution 1","A stack of days still waiting for something warmer, kept in decreasing temperature order. Each new day resolves and pops every colder day below it, so every day is pushed once and popped once — O(n).",`import gleam/dict
import gleam/list
...
}`))}function JL(){return new p(j([["Solution 1",`Compare against the midpoint and throw away the half that cannot hold the answer. O(log n); the only thing to get right is which side the midpoint itself falls on, which is what decides whether the loop terminates.
...
}`))}function QL(){return new p(j([["Solution 1",`Compare against the midpoint and throw away the half that cannot hold the answer. O(log n); the only thing to get right is which side the midpoint itself falls on, which is what decides whether the loop terminates.
...
}`))}function XL(){return new p(j([["Solution 1",`Compare against the midpoint and throw away the half that cannot hold the answer. O(log n); the only thing to get right is which side the midpoint itself falls on, which is what decides whether the loop terminates.
...
}`))}function c3(){return new p(j([["Solution 1","",`pub fn length(items: List(a)) -> Int {
  case items {
    [] -> 0
...
}`))}function n3(){return new p(j([["Solution 1","",`pub fn reverse(items: List(a)) -> List(a) {
  reverse_loop(items, [])
}
...
}`))}function d3(){return new p(j([["Solution 1","",`import gleam/int
import gleam/list
...
}`))}function l3(){return new p(j([["Solution 1","",`import gleam/dict
import gleam/list
import gleam/option
import gleam/string
...
}`))}function s3(){return new p(j([["Solution 1","",`import gleam/int
import gleam/result
...
}`))}function i3(){return new p(j([["Solution 1","",`import gleam/dict
import gleam/option
...
}`))}function r3(){return new p(j([["Solution 1","",`import gleam/list
import gleam/string
...
}`))}function a3(){return new p(j([["Solution 1","",`import gleam/list
import gleam/string
...
}`))}function o3(){return new p(j([["Solution 1","",`pub type Player {
...
}`))}function t3(){return new p(j([["Solution 1","",`import gleam/list
import gleam/set
...
}`))}function EO(Z){if(Z==="nc01_contains_duplicate")return new L(fB());else if(Z==="nc02_valid_anagram")return new L(gB());else if(Z==="nc03_two_sum")return new L(uB());else if(Z==="nc04_group_anagrams")return new L(vB());else if(Z==="nc05_top_k_frequent")return new L(mB());else if(Z==="nc06_product_except_self")return new L(pB());else if(Z==="nc07_longest_consecutive")return new L(cB());else if(Z==="nc08_valid_palindrome")return new L(nB());else if(Z==="nc09_two_sum_sorted")return new L(dB());else if(Z==="nc10_three_sum")return new L(lB());else if(Z==="nc11_container_water")return new L(sB());else if(Z==="nc12_best_time_stock")return new L(iB());else if(Z==="nc13_longest_substring")return new L(rB());else if(Z==="nc14_character_replacement")return new L(aB());else if(Z==="nc15_permutation_in_string")return new L(oB());else if(Z==="nc16_valid_parentheses")return new L(tB());else if(Z==="nc17_min_stack")return new L(eB());else if(Z==="nc18_daily_temperatures")return new L(ZL());else if(Z==="nc19_binary_search")return new L(JL());else if(Z==="nc20_find_min_rotated")return new L(QL());else if(Z==="nc21_search_rotated")return new L(XL());else if(Z==="tip01_list_patterns")return new L(c3());else if(Z==="tip02_tail_recursion")return new L(n3());else if(Z==="tip03_fold")return new L(d3());else if(Z==="tip04_frequency_maps")return new L(l3());else if(Z==="tip05_result_chains")return new L(s3());else if(Z==="tip06_option")return new L(i3());else if(Z==="tip07_string_patterns")return new L(r3());else if(Z==="tip08_pipelines")return new L(a3());else if(Z==="tip09_records")return new L(o3());else if(Z==="tip10_set_dedupe")return new L(t3());else return new V0(void 0)}function Z8(Z,J,Q){return new _8(Z,J,z4(Z),F0(Q.solutions,(X)=>{return new Q7(X[0],X[1],X[2])}),gQ,new a(Q.check),z0)}function CO(){return new k8("Gleam Tips",j([new j1("Idioms",j([Z8("Pattern matching on lists","Implement length and last for List(a) using only case expressions with empty, single-element, and head-tail patterns.",c3()),Z8("Tail recursion with accumulators","Implement reverse and sum as tail-recursive functions: a public wrapper plus a private loop helper carrying an accumulator.",n3()),Z8("fold is the loop","Implement max, count_if, and running_total using only list.fold and list.scan - no hand-written recursion.",d3()),Z8("Frequency maps with dict.upsert","Build word_frequencies(text) returning a Dict(String, Int) of word counts: lowercase, split on spaces, drop empties, then fold with dict.upsert.",l3()),Z8("Result chains with use","Define a Config record and parse_config(host, port, timeout) that chains two int.parse calls with use + result.try and rejects an empty host.",s3()),Z8("Option ergonomics",'Write port_description(config) that reads an optional "port" entry from a Dict and produces a display string via option.from_result, option.map, and option.unwrap - no case expression.',i3()),Z8("String prefix patterns and graphemes",'Implement strip_comment using "# " <> rest string-prefix patterns, and initials(name) using string.to_graphemes with list.filter_map.',r3()),Z8("Pipelines","Write slug(title) as a single |> pipeline: trim, lowercase, split on spaces, drop empty words, join with dashes.",a3()),Z8("Records: labelled args and update syntax","Define a Player record and write new_player (labelled construction), add_points, and level_up using Player(..player, field: value) update syntax.",o3()),Z8("gleam/set for membership and dedupe","Implement dedupe(items) preserving first occurrence: fold carrying #(kept, seen_set), then reverse the kept list.",t3())]))]))}class l0 extends P{constructor(Z,J,Q){super();this.title=Z,this.prompt=J,this.stem=Q}}class I4 extends P{constructor(Z,J){super();this.subcategory=Z,this.drills=J}}var j9="NeetCode 150";function KL(){return j([new I4("Arrays & Hashing",j([new l0("Contains Duplicate","Given an integer array nums, return true if any value appears at least twice in the array, and return false if every element is distinct.","nc01_contains_duplicate"),new l0("Valid Anagram","Given two strings s and t, return true if t is an anagram of s, and false otherwise.","nc02_valid_anagram"),new l0("Two Sum","Given an array of integers nums and an integer target, return indices of the two numbers such that they add up to target.","nc03_two_sum"),new l0("Group Anagrams","Given an array of strings strs, group the anagrams together. You can return the answer in any order.","nc04_group_anagrams"),new l0("Top K Frequent Elements","Given an integer array nums and an integer k, return the k most frequent elements. You may return the answer in any order.","nc05_top_k_frequent"),new l0("Product of Array Except Self","Given an integer array nums, return an array answer such that answer[i] is equal to the product of all the elements of nums except nums[i]. Solve without division and in O(n) time.","nc06_product_except_self"),new l0("Longest Consecutive Sequence","Given an unsorted array of integers nums, return the length of the longest consecutive elements sequence. You must write an algorithm that runs in O(n) time.","nc07_longest_consecutive")])),new I4("Two Pointers",j([new l0("Valid Palindrome","A phrase is a palindrome if, after converting all uppercase letters into lowercase letters and removing all non-alphanumeric characters, it reads the same forward and backward.","nc08_valid_palindrome"),new l0("Two Sum II - Input Array Is Sorted","Given a 1-indexed array of integers numbers that is already sorted in non-decreasing order, find two numbers such that they add up to a specific target number. Return the two indices, 1-indexed.","nc09_two_sum_sorted"),new l0("3Sum","Given an integer array nums, return all the triplets [nums[i], nums[j], nums[k]] such that i, j, and k are distinct and the sum is 0. The solution set must not contain duplicate triplets.","nc10_three_sum"),new l0("Container With Most Water","You are given an integer array height of length n. Find two lines that together with the x-axis form a container that holds the most water. Return the maximum amount of water a container can store.","nc11_container_water")])),new I4("Sliding Window",j([new l0("Best Time to Buy and Sell Stock","You are given an array prices where prices[i] is the price of a given stock on the ith day. Maximize your profit by choosing a single day to buy and a different day in the future to sell.","nc12_best_time_stock"),new l0("Longest Substring Without Repeating Characters","Given a string s, find the length of the longest substring without repeating characters.","nc13_longest_substring"),new l0("Longest Repeating Character Replacement","You are given a string s and an integer k. You can change any character to any other uppercase English character at most k times. Return the length of the longest substring containing the same letter you can get.","nc14_character_replacement"),new l0("Permutation in String","Given two strings s1 and s2, return true if s2 contains a permutation of s1. In other words, return true if one of s1's permutations is a substring of s2.","nc15_permutation_in_string")])),new I4("Stack",j([new l0("Valid Parentheses","Given a string s containing just the characters '(', ')', '{', '}', '[' and ']', determine if the input string is valid.","nc16_valid_parentheses"),new l0("Min Stack","Design a stack that supports push, pop, top, and retrieving the minimum element in constant time.","nc17_min_stack"),new l0("Daily Temperatures","Given an array of integers temperatures, return an array answer such that answer[i] is the number of days you have to wait after the ith day to get a warmer temperature.","nc18_daily_temperatures")])),new I4("Binary Search",j([new l0("Binary Search","Given a sorted array of integers nums and an integer target, write a function to search target in nums. Return its index, or -1 if it does not exist. Must run in O(log n) time.","nc19_binary_search"),new l0("Find Minimum in Rotated Sorted Array","Suppose an array of length n sorted in ascending order is rotated. Given the rotated array of unique elements, return the minimum element. Must run in O(log n) time.","nc20_find_min_rotated"),new l0("Search in Rotated Sorted Array","Given a rotated sorted array of distinct integers nums and an integer target, return the index of target, or -1 if it is not present. Must run in O(log n) time.","nc21_search_rotated")]))])}function qL(Z,J,Q){return F1(Q(Z.stem),(X)=>{let Y=X[0],K=X[1];return new _8(Z.title,Z.prompt,z4(Z.title),F0(Y,(q)=>{return new Q7(q[0],q[1],q[2])}),J,K,z0)})}function H9(Z,J,Q){return new k8(Z,(()=>{let X=KL();return V7(X,(Y)=>{let K=V7(Y.drills,(q)=>{return qL(q,J,Q)});if(K instanceof x)return new V0(void 0);else{let q=K;return new L(new j1(Y.subcategory,q))}})})())}function WL(){return j([["Solution 1","A set answers the whole question. Walk once, and the moment a value is already in the set you are done — no need to finish the pass, and no need to know where the duplicate was.",`defmodule Solution do
...
end`]])}function GL(){return j([["Solution 1","Two strings are anagrams exactly when every character occurs the same number of times in both, so build a count per string and compare the two maps.",`defmodule Solution do
  def anagram?(s, t) do
...
end`]])}function UL(){return j([["Solution 1","Every number seen so far is already in the map, so the complement is one lookup away — one pass, O(1) per step, and the map hands back the index for free.",`defmodule Solution do
...
end`]])}function zL(){return j([["Solution 1",`Bucket by a key that comes out identical for everything that belongs together. Once the key is anagram-invariant the grouping is just a map from key to list, and no pair of words is ever compared directly.
...
end`]])}function IL(){return j([["Solution 1","Count, then select. The frequencies come first; picking the k largest is a separate question, and which method you use for it is what separates the variants.",`defmodule Solution do
...
end`]])}function VL(){return j([["Solution 1","The answer at each slot is everything before it times everything after it. One forward pass builds the prefixes, one reverse pass folds in the suffixes — and no division, so a zero in the input costs nothing special.",`defmodule Solution do
...
end`]])}function jL(){return j([["Solution 1","Put everything in a set, then only start counting at numbers with no predecessor. That guard is what keeps it O(n): every run is walked exactly once instead of once per member.",`defmodule Solution do
...
end`]])}function HL(){return j([["Solution 1","Normalise first — letters and digits only, lowercased — and the palindrome test is whatever comparison you like: two pointers converging, or the cleaned string against its reverse.",`defmodule Solution do
  def palindrome?(s) do
    cleaned = clean(s)
...
end`]])}function OL(){return j([["Solution 1",`Sorted input plus a pointer at each end. A sum that is too small can only be fixed by raising the low end, one that is too large by lowering the high end, so neither pointer ever needs to go back. They meet in O(n) with no extra memory.
...
end`]])}function FL(){return j([["Solution 1","Sort, fix one number, then run the two-pointer scan on the remainder looking for its negation. Sorting is what makes the duplicate triples skippable: equal values are adjacent, so stepping past them is a while loop, not a set.",`defmodule Solution do
...
end`]])}function NL(){return j([["Solution 1","Start at both ends. The area is capped by the shorter line, so moving the taller one in can never help — always move the shorter, and track the best area seen.",`defmodule Solution do
...
end`]])}function RL(){return j([["Solution 1","Carry the cheapest price seen so far; today's best sale is today's price against that minimum. One pass, two variables.",`defmodule Solution do
...
end`]])}function ML(){return j([["Solution 1","Grow a window rightwards and, whenever the new character is already inside it, move the start past that character's earlier copy. The window is always repeat-free, so its widest reading is the answer.",`defmodule Solution do
...
end`]])}function AL(){return j([["Solution 1","A window can be made uniform with k changes when its size minus its most-frequent-character count is at most k. Grow the right edge, shrink from the left when that breaks, and the biggest valid window is the answer.",`defmodule Solution do
...
end`]])}function DL(){return j([["Solution 1","A permutation of s1 is any window of length |s1| in s2 with identical character counts. Slide one character at a time, adding the entering character and removing the leaving one, so each step is O(1) rather than a recount.",`defmodule Solution do
...
end`]])}function BL(){return j([["Solution 1","On every opener, push the closer you expect; on every closer, it must match the top. Valid means never mismatching and finishing with an empty stack — both halves are needed.",`defmodule Solution do
  @closers %{"(" => ")", "[" => "]", "{" => "}"}
...
end`]])}function LL(){return j([["Solution 1","The minimum has to be O(1), so it cannot be computed on demand — it has to be carried. Either each entry remembers the minimum at or below it, or a second stack tracks the running minimum alongside the first.",`defmodule Solution do
...
end`]])}function PL(){return j([["Solution 1","A stack of days still waiting for something warmer, kept in decreasing temperature order. Each new day resolves and pops every colder day below it, so every day is pushed once and popped once — O(n).",`defmodule Solution do
...
end`]])}function TL(){return j([["Solution 1",`Compare against the midpoint and throw away the half that cannot hold the answer. O(log n); the only thing to get right is which side the midpoint itself falls on, which is what decides whether the loop terminates.
...
end`]])}function SL(){return j([["Solution 1",`Compare against the midpoint and throw away the half that cannot hold the answer. O(log n); the only thing to get right is which side the midpoint itself falls on, which is what decides whether the loop terminates.
...
end`]])}function EL(){return j([["Solution 1",`Compare against the midpoint and throw away the half that cannot hold the answer. O(log n); the only thing to get right is which side the midpoint itself falls on, which is what decides whether the loop terminates.
...
end`]])}function _O(Z){if(Z==="nc01_contains_duplicate")return new L(WL());else if(Z==="nc02_valid_anagram")return new L(GL());else if(Z==="nc03_two_sum")return new L(UL());else if(Z==="nc04_group_anagrams")return new L(zL());else if(Z==="nc05_top_k_frequent")return new L(IL());else if(Z==="nc06_product_except_self")return new L(VL());else if(Z==="nc07_longest_consecutive")return new L(jL());else if(Z==="nc08_valid_palindrome")return new L(HL());else if(Z==="nc09_two_sum_sorted")return new L(OL());else if(Z==="nc10_three_sum")return new L(FL());else if(Z==="nc11_container_water")return new L(NL());else if(Z==="nc12_best_time_stock")return new L(RL());else if(Z==="nc13_longest_substring")return new L(ML());else if(Z==="nc14_character_replacement")return new L(AL());else if(Z==="nc15_permutation_in_string")return new L(DL());else if(Z==="nc16_valid_parentheses")return new L(BL());else if(Z==="nc17_min_stack")return new L(LL());else if(Z==="nc18_daily_temperatures")return new L(PL());else if(Z==="nc19_binary_search")return new L(TL());else if(Z==="nc20_find_min_rotated")return new L(SL());else if(Z==="nc21_search_rotated")return new L(EL());else return new V0(void 0)}function kO(){return H9(j9+" (Elixir)",LO,(Z)=>{return F1(_O(Z),(J)=>{return[J,z0]})})}function wO(){return H9(j9+" (Gleam)",gQ,(Z)=>{return F1(EO(Z),(J)=>{return[J.solutions,new a(J.check)]})})}function wL(){return new p(j([["Solution 1","A set answers the whole question. Walk once, and the moment a value is already in the set you are done — no need to finish the pass, and no need to know where the duplicate was.",`def containsDuplicate(nums):
    seen = set()
    for num in nums:
        if num in seen:
            return True
...
__case__("containsDuplicate([])", False, containsDuplicate([]))`))}function xL(){return new p(j([["Solution 1","Two strings are anagrams exactly when every character occurs the same number of times in both, so build a count per string and compare the two maps.",`def isAnagram(s, t):
    if len(s) != len(t):
        return False
...
__case__("isAnagram('a', 'ab')", False, isAnagram("a", "ab"))`))}function bL(){return new p(j([["Solution 1","Every number seen so far is already in the map, so the complement is one lookup away — one pass, O(1) per step, and the map hands back the index for free.",`def twoSum(nums, target):
    seen = {}
...
__case__("twoSum([1, 2], 7)", [], twoSum([1, 2], 7))`))}function yL(){return new p(j([["Solution 1",`Bucket by a key that comes out identical for everything that belongs together. Once the key is anagram-invariant the grouping is just a map from key to list, and no pair of words is ever compared directly.
...
__case__("groupAnagrams(['a'])", [["a"]], sorted(groupAnagrams(["a"])))`))}function hL(){return new p(j([["Solution 1","Count, then select. The frequencies come first; picking the k largest is a separate question, and which method you use for it is what separates the variants.",`from collections import Counter
...
__case__("productExceptSelf([2, 3])", [3, 2], productExceptSelf([2, 3]))`))}function fL(){return new p(j([["Solution 1","Put everything in a set, then only start counting at numbers with no predecessor. That guard is what keeps it O(n): every run is walked exactly once instead of once per member.",`def longestConsecutive(nums):
    num_set = set(nums)
    longest = 0
...
__case__("longestConsecutive([])", 0, longestConsecutive([]))`))}function gL(){return new p(j([["Solution 1","Normalise first — letters and digits only, lowercased — and the palindrome test is whatever comparison you like: two pointers converging, or the cleaned string against its reverse.",`def isPalindrome(s):
...
__case__("threeSum([0, 0, 0])", [[0, 0, 0]], threeSum([0, 0, 0]))`))}function mL(){return new p(j([["Solution 1","Start at both ends. The area is capped by the shorter line, so moving the taller one in can never help — always move the shorter, and track the best area seen.",`def maxArea(height):
...
__case__("maxArea([1, 1])", 1, maxArea([1, 1]))`))}function pL(){return new p(j([["Solution 1","Carry the cheapest price seen so far; today's best sale is today's price against that minimum. One pass, two variables.",`def maxProfit(prices):
    lowest = float('inf')
    profit = 0
...
__case__("maxProfit([])", 0, maxProfit([]))`))}function cL(){return new p(j([["Solution 1","Grow a window rightwards and, whenever the new character is already inside it, move the start past that character's earlier copy. The window is always repeat-free, so its widest reading is the answer.",`def lengthOfLongestSubstring(s):
    window = set()
    left = 0
    longest = 0
...
__case__("checkInclusion('adc', 'dcda')", True, checkInclusion("adc", "dcda"))`))}function lL(){return new p(j([["Solution 1","On every opener, push the closer you expect; on every closer, it must match the top. Valid means never mismatching and finishing with an empty stack — both halves are needed.",`def isValid(s):
    pairs = {')': '(', ']': '[', '}': '{'}
    stack = []
...
__case__("isValid('([)]')", False, isValid("([)]"))
__case__("isValid('{[]}')", True, isValid("{[]}"))
__case__("isValid('(')", False, isValid("("))`))}function sL(){return new p(j([["Solution 1","The minimum has to be O(1), so it cannot be computed on demand — it has to be carried. Either each entry remembers the minimum at or below it, or a second stack tracks the running minimum alongside the first.",`class MinStack:
    def __init__(self):
        self.stack = []
        self.mins = []
...
__case__("search([], 1)", -1, search([], 1))`))}function aL(){return new p(j([["Solution 1",`Compare against the midpoint and throw away the half that cannot hold the answer. O(log n); the only thing to get right is which side the midpoint itself falls on, which is what decides whether the loop terminates.
...
__case__("findMin([2, 1])", 1, findMin([2, 1]))`))}function oL(){return new p(j([["Solution 1",`Compare against the midpoint and throw away the half that cannot hold the answer. O(log n); the only thing to get right is which side the midpoint itself falls on, which is what decides whether the loop terminates.
...
__case__("search([4, 5, 6, 7, 0, 1, 2], 6)", 2, search([4, 5, 6, 7, 0, 1, 2], 6))`))}function e3(){return new p(j([["Solution 1","",`from collections import Counter
...
__case__("countOf([1, 1, 2], 9)", 0, countOf([1, 1, 2], 9))`))}function Zq(){return new p(j([["Solution 1","",`from collections import defaultdict
...
__case__("groupByLength([])", {}, groupByLength([]))`))}function Jq(){return new p(j([["Solution 1","",`from collections import deque
...
__case__("bfsOrder({'x': []}, 'x')", ["x"], bfsOrder({"x": []}, "x"))`))}function Qq(){return new p(j([["Solution 1","",`import heapq
...
__case__("kSmallest([3], 5)", [3], kSmallest([3], 5))`))}function Xq(){return new p(j([["Solution 1","",`def firstIndexOf(nums, target):
...
__case__("dotProduct([1, 2, 3], [4, 5, 6])", 32, dotProduct([1, 2, 3], [4, 5, 6]))`))}function Yq(){return new p(j([["Solution 1","",`def reversedString(s):
    return s[::-1]
...
__case__("trimEnds('~mid~')", "mid", trimEnds("~mid~"))`))}function Kq(){return new p(j([["Solution 1","",`def sortByLength(words):
...
__case__("sortPairs([('b', 1), ('a', 1), ('a', 9)])", [("a", 9), ("a", 1), ("b", 1)], sortPairs([("b", 1), ("a", 1), ("a", 9)]))`))}function qq(){return new p(j([["Solution 1","",`def joinUpper(chars):
    parts = []
    for c in chars:
...
__case__("joinUpper([])", "", joinUpper([]))`))}function xO(Z){if(Z==="nc01_contains_duplicate")return new L(wL());else if(Z==="nc02_valid_anagram")return new L(xL());else if(Z==="nc03_two_sum")return new L(bL());else if(Z==="nc04_group_anagrams")return new L(yL());else if(Z==="nc05_top_k_frequent")return new L(hL());else if(Z==="nc06_product_except_self")return new L($L());else if(Z==="nc07_longest_consecutive")return new L(fL());else if(Z==="nc08_valid_palindrome")return new L(gL());else if(Z==="nc09_two_sum_sorted")return new L(uL());else if(Z==="nc10_three_sum")return new L(vL());else if(Z==="nc11_container_water")return new L(mL());else if(Z==="nc12_best_time_stock")return new L(pL());else if(Z==="nc13_longest_substring")return new L(cL());else if(Z==="nc14_character_replacement")return new L(nL());else if(Z==="nc15_permutation_in_string")return new L(dL());else if(Z==="nc16_valid_parentheses")return new L(lL());else if(Z==="nc17_min_stack")return new L(sL());else if(Z==="nc18_daily_temperatures")return new L(iL());else if(Z==="nc19_binary_search")return new L(rL());else if(Z==="nc20_find_min_rotated")return new L(aL());else if(Z==="nc21_search_rotated")return new L(oL());else if(Z==="tip01_counter")return new L(e3());else if(Z==="tip02_defaultdict")return new L(Zq());else if(Z==="tip03_deque")return new L(Jq());else if(Z==="tip04_heapq")return new L(Qq());else if(Z==="tip05_enumerate_zip")return new L(Xq());else if(Z==="tip06_slicing")return new L(Yq());else if(Z==="tip07_sort_key")return new L(Kq());else if(Z==="tip08_join")return new L(qq());else return new V0(void 0)}function yO(){return H9(j9,$Q,(Z)=>{return F1(xO(Z),(J)=>{return[J.solutions,new a(J.check)]})})}function eL(){return new p(j([["Solution 1","A set answers the whole question. Walk once, and the moment a value is already in the set you are done — no need to finish the pass, and no need to know where the duplicate was.",`export function containsDuplicate(nums: number[]): boolean {
...
}`))}function ZP(){return new p(j([["Solution 1","Two strings are anagrams exactly when every character occurs the same number of times in both, so build a count per string and compare the two maps.",`export function isAnagram(s: string, t: string): boolean {
...
}`))}function JP(){return new p(j([["Solution 1","Every number seen so far is already in the map, so the complement is one lookup away — one pass, O(1) per step, and the map hands back the index for free.",`export function twoSum(nums: number[], target: number): number[] {
...
}`))}function QP(){return new p(j([["Solution 1",`Bucket by a key that comes out identical for everything that belongs together. Once the key is anagram-invariant the grouping is just a map from key to list, and no pair of words is ever compared directly.
...
}`))}function XP(){return new p(j([["Solution 1","Count, then select. The frequencies come first; picking the k largest is a separate question, and which method you use for it is what separates the variants.",`export function topKFrequent(nums: number[], k: number): number[] {
...
}`))}function KP(){return new p(j([["Solution 1","Put everything in a set, then only start counting at numbers with no predecessor. That guard is what keeps it O(n): every run is walked exactly once instead of once per member.",`export function longestConsecutive(nums: number[]): number {
...
}`))}function qP(){return new p(j([["Solution 1","Normalise first — letters and digits only, lowercased — and the palindrome test is whatever comparison you like: two pointers converging, or the cleaned string against its reverse.",`export function isPalindrome(s: string): boolean {
...
}`))}function WP(){return new p(j([["Solution 1",`Sorted input plus a pointer at each end. A sum that is too small can only be fixed by raising the low end, one that is too large by lowering the high end, so neither pointer ever needs to go back. They meet in O(n) with no extra memory.
...
}`))}function UP(){return new p(j([["Solution 1","Start at both ends. The area is capped by the shorter line, so moving the taller one in can never help — always move the shorter, and track the best area seen.",`export function maxArea(height: number[]): number {
  let left = 0;
...
}`))}function zP(){return new p(j([["Solution 1","Carry the cheapest price seen so far; today's best sale is today's price against that minimum. One pass, two variables.",`export function maxProfit(prices: number[]): number {
  let lowest = Infinity;
  let profit = 0;
...
}`))}function IP(){return new p(j([["Solution 1","Grow a window rightwards and, whenever the new character is already inside it, move the start past that character's earlier copy. The window is always repeat-free, so its widest reading is the answer.",`export function lengthOfLongestSubstring(s: string): number {
...
}`))}function HP(){return new p(j([["Solution 1","On every opener, push the closer you expect; on every closer, it must match the top. Valid means never mismatching and finishing with an empty stack — both halves are needed.",`export function isValid(s: string): boolean {
...
}`))}function OP(){return new p(j([["Solution 1","The minimum has to be O(1), so it cannot be computed on demand — it has to be carried. Either each entry remembers the minimum at or below it, or a second stack tracks the running minimum alongside the first.",`export class MinStack {
...
}`))}function FP(){return new p(j([["Solution 1","A stack of days still waiting for something warmer, kept in decreasing temperature order. Each new day resolves and pops every colder day below it, so every day is pushed once and popped once — O(n).",`export function dailyTemperatures(temperatures: number[]): number[] {
...
}`))}function NP(){return new p(j([["Solution 1",`Compare against the midpoint and throw away the half that cannot hold the answer. O(log n); the only thing to get right is which side the midpoint itself falls on, which is what decides whether the loop terminates.
...
}`))}function RP(){return new p(j([["Solution 1",`Compare against the midpoint and throw away the half that cannot hold the answer. O(log n); the only thing to get right is which side the midpoint itself falls on, which is what decides whether the loop terminates.
...
}`))}function MP(){return new p(j([["Solution 1",`Compare against the midpoint and throw away the half that cannot hold the answer. O(log n); the only thing to get right is which side the midpoint itself falls on, which is what decides whether the loop terminates.
